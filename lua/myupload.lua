...
-- 文件保存的根路径
local saveRootPath = "/home/nginx/openresty/nginx/userfiles/upload/"
...
while true do
    ...
    if typ == "header" then
        ...
        if key == "Content-Disposition" then
			...
			if filename then
			 local kvex = string.split(filename, ".")
			 local ex = "."..kvex[table.getn(kvex)]
		         local filep = os.time()- 1495535000
			  filename = filep..ex
